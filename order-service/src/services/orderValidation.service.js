...
export const validateOrderAsync = async (orderId, previousItems = null) => {
    try{
        ...
        console.log(`Published inventory reserve event for order ${orderId}`);
    }catch (error){
        console.error('Error publishing inventory reserve event', error);
        await Order.findByIdAndUpdate(orderId, {
            status: 'Failed',
            failureReason: 'System error'
        });
        ...
        try{
            ...
            console.error('Error publishing order failed event', kafkaError);
        }
    }
}
...
export const restoreOrderAsync = async (orderId) => {
    try {
        ...
        await producer.send({
            topic: TOPICS.INVENTORY_RESTORE,
            messages: [{
                key: orderId.toString(),
                value: JSON.stringify({
                    orderId: orderId.toString(),
        ...
        console.log(`Published inventory restore event for order ${orderId}`);
    } catch (error) {
        console.error('Error publishing inventory restore event', error);
    }
}
